...
#### Calculations for getting a few predictions from the Sloutsky & Fisher model.

Sloutsky & Fisher's lovely model presumes that induction judgments are based on the relative similarity of a Target picture to a picture X, and to a picture Y. As they show, this can be reduced to p(Y) = 1/(1 + (W * S_X) / W * S_Y), where W is the weight given by a shared/unshared word, S_X is similarity of X to target, and S_Y is similarity of Y to target. 

Assume that if Y and target share a label then W = 1, and that if X and Target do not share a label then W = 0.1 (following S&F2004).
When Y shares a label and  similarity ratio = 1 then p(Y) = 1/(1+((0.1\*0.5)/0.5)) = `r round(1/(1+((0.1*0.5)/0.5)),2)`, but when Y does not share a label then p(Y) =  1/(1+((1\*0.5)/0.5)) = `r round(1/(1+((1*0.5)/0.5)),2)`

If similarity ratio = 10 then shared label p(Y) = 1/(1+((0.1\*0.5)/0.05)) = `r round(1/(1+((0.1*0.5)/0.05)),2)` while non-shared label p(Y) = 1/(1+((1\*0.5)/0.05)) = `r round(1/(1+((1*0.5)/0.05)),2)`.

If similarity ratio = 100 then shared label p(Y) = 1/(1+((0.1 \* 0.5)/0.005)) = `r round(1/(1+((0.1*0.5)/0.005)),2)` while non-shared label p(Y) = 1/(1+((1\*0.5)/0.005)) = `r round(1/(1+((1*0.5)/0.005)),2)`.

Adding a label still provides a 10%age point increase in p(Y) if the Y picture is `r 0.5/0.0064` times less similar to the target than the X picture. (code: 1/(1+((0.1\*0.5)/0.0064)) - 1/(1+((1\*0.5)/0.0064)) which produces a `r 100*round(1/(1+((0.1*0.5)/0.0064)) - 1/(1+((1*0.5)/0.0064)),2)` percentage point increase in probability of choosing Y.) 

Finally, in the discussion section of the paper we note that we can estimate the relative similarity of pictures in the Different Sense condition by looking at the proportion of trials on which participants choose the Dissimilar Test picture in the No Label condition of Experiment 1. 

* p(y) = 0.27 = 1/ (1+((1\*x)/1\*y))
* 0.27 \* (1+((1\*x)/1\*y)) = 1
* 0.27 \* (1+(ratio)) = 1
* 1+(ratio) = 1/0.27
* ratio = 1/0.27 - 1 = `r 1/0.27 -1`

We can use this value to calculate what the predicted effect of shared label should be in this case. 

If the Y picture is `r round(0.5/0.1851,2)` times less similar to the target than the X picture. (code: 1/(1+((0.1\*0.5)/0.1851)) - 1/(1+((1\*0.5)/0.1851)) then adding a shared label produces a `r 100*round(1/(1+((0.1*0.5)/0.1851)) - 1/(1+((1*0.5)/0.1851)),2)` percentage point increase in probability of choosing Y, from `r round(1/(1+((1*0.5)/0.1851)),2)` to `r round(1/(1+((0.1*0.5)/0.1851)),2)`.
...
#add error bars
avg.plot+geom_errorbar(aes(ymax=Adult.means$Choice.mean+Adult.means$SE, ymin=Adult.means$Choice.mean-Adult.means$SE), position=position_dodge(.9), width = 0)+theme_bw()+ylab("Proportion adults choosing dissimilar test picture")+ylim(c(-0.05,1))+ geom_hline(yintercept = 0.5, colour = "black",lty = 3)+xlab("")
...
avg.plot<- ggplot(Adult.means, aes(Meaning, Choice.mean, fill = Label)) + 
 stat_summary(fun.y="mean", geom="bar", position = "dodge")
#add error bars
avg.plot+geom_errorbar(aes(ymax=Adult.means$Choice.mean+Adult.means$SE, ymin=Adult.means$Choice.mean-Adult.means$SE), position=position_dodge(.9), width = 0)+theme_bw()+ylab("Proportion adults choosing dissimilar test picture")+ylim(c(-0.05,1.05))+ geom_hline(yintercept = 0.5, colour = "black",lty = 3)+xlab("")+facet_grid(.~triad.type)+ scale_x_discrete(breaks=levels(Adult.means$Meaning),
...
kable(summaryBy(Choice ~ Meaning + Label, data = Adult.Sum, FUN = c(mean,sd)) )
...
avg.plot<- ggplot(Child.means, aes(Meaning, Choice.mean, fill = Label)) + 
 stat_summary(fun.y="mean", geom="bar", position = "dodge")
#add error bars
avg.plot+geom_errorbar(aes(ymax=Child.means$Choice.mean+Child.means$SE, ymin=Child.means$Choice.mean-Child.means$SE), position=position_dodge(.9), width = 0)+theme_bw()+ylab("Proportion children choosing dissimilar test picture")+ylim(c(0,1))+ geom_hline(yintercept = 0.5, colour = "black",lty = 3)+xlab("")
...
kable(summaryBy(Choice ~ Meaning + Label, data = Child.Sum, FUN = c(mean,sd)) )
...
avg.plot<- ggplot(Adult.means, aes(Meaning, Choice.mean, fill = Label)) + 
 stat_summary(fun.y="mean", geom="bar", position = "dodge")
#add error bars
avg.plot+geom_errorbar(aes(ymax=Adult.means$Choice.mean+Adult.means$SE, ymin=Adult.means$Choice.mean-Adult.means$SE), position=position_dodge(.9), width = 0)+theme_bw()+ylab("Proportion adults choosing dissimilar test picture")+ylim(c(0,1))+ geom_hline(yintercept = 0.5, colour = "black",lty = 3)+xlab("")
...
kable(summaryBy(Choice ~ WordType + LabelType, data = Adult.Sum, FUN = c(mean,sd)) )
...
ttestBF(subset(Adult.Sum, WordType == "Polysemous" & LabelType == "Same")$Choice,subset(Adult.Sum, WordType == "Polysemous" & LabelType == "Different")$Choice, paired = T)
...
print(paste("Tested in Edinb = ", length(unique(subset(Child,  Loc != "Ber")$SubjNo))))

print(paste("Tested in Berkeley = ", length(unique(subset(Child,  Loc == "Ber")$SubjNo))))
...
avg.plot<- ggplot(Child.means, aes(Meaning, Choice.mean, fill = Label)) + 
 stat_summary(fun.y="mean", geom="bar", position = "dodge")
#add error bars
avg.plot+geom_errorbar(aes(ymax=Child.means$Choice.mean+Child.means$SE, ymin=Child.means$Choice.mean-Child.means$SE), position=position_dodge(.9), width = 0)+theme_bw()+ylab("Proportion children choosing dissimilar test picture")+ylim(c(0,0.75))+ geom_hline(yintercept = 0.5, colour = "black",lty = 3)+xlab("")
...
avg.plot<- ggplot(Child.means, aes(Meaning, Choice.mean, fill = Label)) + 
 stat_summary(fun.y="mean", geom="bar", position = "dodge")
#add error bars
avg.plot+geom_errorbar(aes(ymax=Child.means$Choice.mean+Child.means$SE, ymin=Child.means$Choice.mean-Child.means$SE), position=position_dodge(.9), width = 0)+theme_bw()+ylab("Proportion children choosing dissimilar test picture")+ylim(c(0,0.75))+ geom_hline(yintercept = 0.5, colour = "black",lty = 3)+xlab("")+facet_grid(.~TriadType)+ scale_x_discrete(breaks=levels(Child.means$Meaning),
                      labels=c("Unambiguous", "Flexible"))
...
kable(summaryBy(Choice ~ WordType + LabelType, data = Child.Sum, FUN = c(mean,sd)) )
...
ttestBF(subset(Child.Sum, WordType == "Polysemous" & LabelType == "Same")$Choice,subset(Child.Sum, WordType == "Polysemous" & LabelType == "Different")$Choice, paired = T)
...
avg.plot<- ggplot(Adult.means, aes(Meaning, Choice.mean, fill = Label_c)) + 
 stat_summary(fun.y="mean", geom="bar", position = "dodge")
#add error bars
avg.plot+geom_errorbar(aes(ymax=Adult.means$Choice.mean+Adult.means$SE, ymin=Adult.means$Choice.mean-Adult.means$SE), position=position_dodge(.9), width = 0)+theme_bw()+ylab("Proportion adults choosing dissimilar test picture")+ylim(c(0,1.01))+ geom_hline(yintercept = 0.5, colour = "black",lty = 3)+xlab("")+facet_grid(.~Expt)+ scale_x_discrete(breaks=levels(Adult.means$Meaning),
                      labels=c("Unambiguous", "Flexible"))
```
And data analysis
...
avg.plot<- ggplot(Child.means, aes(Meaning, Choice.mean, fill = Label_c)) + 
 stat_summary(fun.y="mean", geom="bar", position = "dodge")
#add error bars
avg.plot+geom_errorbar(aes(ymax=Child.means$Choice.mean+Child.means$SE, ymin=Child.means$Choice.mean-Child.means$SE), position=position_dodge(.9), width = 0)+theme_bw()+ylab("Proportion children choosing dissimilar test picture")+ylim(c(0,1.01))+ geom_hline(yintercept = 0.5, colour = "black",lty = 3)+xlab("")+facet_grid(.~Expt)+ scale_x_discrete(breaks=levels(Child.means$Meaning),
                      labels=c("Unambiguous", "Flexible"))
```
